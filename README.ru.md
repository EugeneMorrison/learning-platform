...
│           ├── RichTextEditor.jsx     # WYSIWYG-редактор (TipTap) для TEXT-блоков / условий CODE
...
* **Добавить** блок любого типа — 📝 Теория (TEXT), ❓ Тест (QUIZ), 💻 Задача на код (CODE)
...
|------|--------------------------|
| TEXT | **WYSIWYG-редактор** (TipTap) — жирный, курсив, H2/H3, маркированные и нумерованные списки, строчный код, блок кода, цитата. Преподаватель не пишет HTML вручную; редактор сам формирует `{"html": ...}`, который сохраняется в блок. Блоки кода становятся `<pre><code>` и автоматически подсвечиваются в просмотре урока. |
| QUIZ | Поле вопроса, динамический список вариантов с переключателем для отметки правильного и пояснение, которое показывается при верном ответе. Пустые варианты удаляются при сохранении, а `correct_answer` автоматически переиндексируется. |
...
* Файлы: `frontend/src/pages/LessonEditor.jsx`, `frontend/src/components/RichTextEditor.jsx` (+ `.css`).
* **Бэкенд не пришлось дописывать** — редактор использует существующий DRF `BlockViewSet` (`POST` / `PUT` / `PATCH` / `DELETE /api/blocks/`). `BlockSerializer` проверяет обязательные поля по типу блока и возвращает `400` при некорректном содержимом. Доступ только для автора через существующие права `IsAuthor` / `IsOwnerOrReadOnly`.
* **Порядок и уникальность:** у `Block` есть ограничение `unique_together (lesson, order_index)`. Новые блоки получают `max(order_index) + 1` (а не «количество + 1»), поэтому пропуски после удалений не вызывают конфликтов. Перестановка меняет местами два блока через временный свободный индекс, чтобы не нарушить ограничение в процессе обмена. Создание урока на странице курса использует то же правило `max + 1`.
